...
- **iOS / watchOS (simpulse-mac, Xcode 26.6):** `xcodebuild test` scheme SimPulse iPhone 17 — **TEST SUCCEEDED** (8 tests: 4 glance presentation + 4 WorkoutSessionController). `build-watch.sh` — **BUILD SUCCEEDED**. CODE_SIGNING_ALLOWED=NO. Physical Watch Always On not run.
...
| `xcodebuild test` SimPulse | simpulse-mac, iPhone 17 simulator | **8 passed**, 0 failed (2026-08-18, WATCH-002) |
...
1. WATCH-003 persist/sync summary to iPhone (needs IOS-004)
2. IOS-001 session history
