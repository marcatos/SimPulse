...
- iRacing: first-party mmap session reader (BRIDGE-003 + BUG-001 DONE). BRIDGE-008 Task 1 adds official header layout + typed var-table reads (`SessionTime`, `SessionNum`, `DriverCarIdx`, `Lap`) from synthetic buffers. Task 2 resolves vehicle/session from YAML `Drivers`/`Sessions` lists; unmatched `driverCarIdx`/`sessionNum` are `Unavailable` (no first-entry fallback). Live YAML / live 60 Hz still requires the sim + memmap (KI-002). Adapter wiring is Task 3. BUG-004 re-runs `Parse` on cached YAML when Available `SessionNum`/`DriverCarIdx` identity changes without a `SessionInfoUpdate` tick.
...
- BUG-004 DONE: cached-YAML identity re-resolve when `SessionInfoUpdate` is unchanged.
...
- **.NET Bridge host:** `dotnet test SimPulse.sln --configuration Release` — succeeded, 0 warnings, 0 errors (2026-08-18, Windows 10.0.26200, SDK 8.0.424). BRIDGE-008 Task 1–2 parsers plus BUG-004 cached-YAML identity re-resolve are covered. BRIDGE-007 host is `net8.0-windows` `WinExe` with `UseWindowsForms`; Linux stays `net8.0` `Exe`. File logs default to `%LOCALAPPDATA%\SimPulse\logs`.
...
| --- | --- | --- |
| `dotnet test SimPulse.sln --configuration Release` | Windows 10.0.26200, SDK 8.0.424 | **126 passed**, 0 failed (Domain 6, Analytics 9, Protocol 7, Bridge.Core 104) |
