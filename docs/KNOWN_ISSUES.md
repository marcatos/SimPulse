...
- **Workaround:** On Windows, read the PIN from the tray balloon or **Show current PIN** (does not rotate the PIN). WinExe file logs are under `%LOCALAPPDATA%\SimPulse\logs` (`SIMPULSE_LOG_DIR` / `SIMPULSE_LOG_FILE=0`). Persist trusted devices with `SIMPULSE_TRUSTED_DEVICES_PATH`. For console logs while debugging, `dotnet run --property:OutputType=Exe` (see `docs/DEVELOPMENT.md`). Keep Bridge on loopback until TLS exists.
